...
// all in us; 400 pulses/rotation in full-step mode
//
//     pulse | time/rotation
// min. 2000 | 0.8s
//      2500 | 1.0s
// max  5000 | 2.0s 
long int pulse = 3750;  // time for one pulse (pulse+pause)

long int ppulse = 100;  // pulse width, 100us is enough
long int npulse = pulse - ppulse; // waittime till whole pulse is finished
int direction = LOW;  // CCW
...
void ignition()
{
  ...
  for (int i=0; i<20; i++)
  {
    digitalWrite(STEP, HIGH); delayMicroseconds(ppulse);
	  digitalWrite(STEP, LOW);  delayMicroseconds(npulse);
  }
  ...
  {
    digitalWrite(STEP, HIGH); delayMicroseconds(ppulse);
	  digitalWrite(STEP, LOW);  delayMicroseconds(npulse*2);
  }
